...
module User::DealsExtension
  ...
  def begin_with(summary_key, limit = nil)
    ...
    results = find(:all, 
      :select => "summary, max(date) as date",
      :conditions => ["summary like ?", "#{summary_key}%"],
      :group => "summary",
      :limit => limit)
    result.empty? ? [] : find(:all, :conditions => [results.map{|r| "(summary = ? and date = ?)"}.join(" or ")].concat(results.map{|r| [r.summary, r.date]}.flatten))
  end
  ...
  def in_range(start_date, end_date, accounts)
    ...
    find(:all,
      :select => "distinct dl.*",
      :conditions => ["et.account_id in (?) and dl.date >= ? and dl.date < ?",
        accounts.map{|a| a.id},
        start_date,
        end_date +1 ],
